...
export class Quimica {
  ...
  verificarResposta(atividade: Atividade) {
    ...
    if (userId) {
      const payload = { usuario_id: userId, materia: 'Química', progresso: progress, pontos: Math.round(progress), faltas: 0, boletim: null };
      this.progressoService.salvar(payload).subscribe({ next: () => console.log('Progresso química salvo'), error: (e) => console.error('Erro salvar quimica:', e) });
    }
  }
  ...
  salvarProgress() {
    ...
    const payload = { usuario_id: userId, materia: 'Química', progresso: progressoAtual, pontos: Math.round(progressoAtual), faltas: 0, boletim: null };
